...
public class BicycleEntity extends baseBike2Entity {

	public BicycleEntity(EntityType<? extends baseBike2Entity> entityType, Level level) {
		...
		if (!level().isClientSide()) {
			if (itemstack.getItem() == Revice_Rider_Items.REX_VISTAMP.get()) {
				if (this.level() instanceof ServerLevel) {
					...
					ViceBikeEntity boss = MobsCore.VICE_BIKE.get().create(this.level());
					if (boss != null) {
